...
def main():
    st.set_page_config(
        page_title="MCP Client Test Console",
        page_icon="🔧",
        layout="wide"
    )
    ...
    with st.sidebar:
        ...
        if connection_type == "stdio":
            ...
            url = st.text_input("SSE URL:", value="http://localhost:8000/sse", key="sse_url")
            ...
            base_url = st.text_input("Base URL:", value="http://localhost:8000", key="http_url")
        ...
        if st.button("Connect"):
            ...
            with st.spinner("Connecting..."):
                try:
                    server_info = asyncio.run(st.session_state.mcp_service.connect(connection_config))
                        ...
                        if tool.input_schema:
                            ...
                            if submitted:
                                with st.spinner("Executing tool..."):
                                    try:
                                        result = asyncio.run(service.execute_tool(tool.name, args))
                                                    ...
                                                    for content_item in result.result.content:
                                                        if hasattr(content_item, 'type'):
                                                            if content_item.type == 'text':
                                                                ...
                                                                st.json(content_item.dict() if hasattr(content_item, 'dict') else str(content_item))
                                                        else:
                                                            st.write(str(content_item))
                                                else:
                                                    st.json(result.result.dict() if hasattr(result.result, 'dict') else str(result.result))
                                        else:
                                            st.warning("⚠️ Tool executed but returned no result")
